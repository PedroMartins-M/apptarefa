...
class DatabaseHelper {
  ...
  static Future<void> inserirTarefa(String titulo) async {
    ...
    await db.insert('tarefas', {
      'titulo': titulo,
      'situacao': 0, //0 = False, 1 = Verdadeiro (SQLite não tem tipo Boolean)
    });
  }
 
  //UPDATE: Alterar o campo marcado
  static Future<void> atualizarSituacao(int id, int situacao) async {
    final db = await DatabaseHelper.database;
    await db.update(
      'tarefas',
      {'situacao': situacao == 1 ? 1 : 0},
      where: 'id = ?',
      whereArgs: [id],
    );
  }
 
  // DELETE: Remover tarefa do banco de dados
